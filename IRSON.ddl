...
CREATE TABLE INTERNATIONAL_FEDERATION (
    id SERIAL NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT federation_fk FOREIGN KEY (id) REFERENCES FEDERATION (id) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE COACH (
    ...
    CONSTRAINT person_fk FOREIGN KEY (ssn) REFERENCES PERSON (ssn) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE DONATION (
    ...
    CONSTRAINT team_fk FOREIGN KEY (club_id) REFERENCES SPORT_TEAM (id) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE TEAM_ROSTER (
    ...
    CONSTRAINT roster_date_check CHECK ( valid_date_range(start_date, end_date) ),
    CONSTRAINT sportsperson_fk FOREIGN KEY (player_ssn) REFERENCES SPORTSPERSON (ssn) ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT team_fk FOREIGN KEY (team_id) REFERENCES SPORT_TEAM (id) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
BEGIN
    ...
    WHERE tf.team_id = NEW.team_id
        AND (tf.end_date IS NULL OR NEW.start_date <= tf.end_date)
        AND NEW.start_date >= tf.start_date
    ;
